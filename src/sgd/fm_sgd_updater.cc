...
DMLC_REGISTER_PARAMETER(FMSGDUpdaterParam);

KWArgs FMSGDUpdater::Init(const KWArgs& kwargs) {
  return param_.InitAllowUnknown(kwargs);
}

void FMSGDUpdater::Evaluate(sgd::Progress* prog) const {
  real_t objv = 0;
  size_t nnz = 0;
...
void FMSGDUpdater::Get(const SArray<feaid_t>& fea_ids,
                     int val_type,
...
void FMSGDUpdater::Update(const SArray<feaid_t>& fea_ids,
                        int value_type,
...
void FMSGDUpdater::UpdateW(real_t gw, FMSGDEntry* e) {
  real_t sg = e->sqrt_g;
  real_t w = e->w;
  // update sqrt_g
  gw += w * param_.l2;
...
void FMSGDUpdater::UpdateV(real_t const* gV, FMSGDEntry* e) {
  int n = param_.V_dim;
  for (int i = 0; i < n; ++i) {
...
void FMSGDUpdater::InitV(FMSGDEntry* e) {
  int n = param_.V_dim;
  e->V = new real_t[n*2];
  for (int i = 0; i < n; ++i) {
